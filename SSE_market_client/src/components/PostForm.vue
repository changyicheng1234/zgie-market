      ...
      <b-col md='8' offset-md='2' lg='10' offset-lg='1'>
        <b-card style='max-width: 1200px;max-height: 2000px;'>
          <b-form-group label='标题'>
            ...
            @click="showEmojiOnTitle()"
            class="btn btn-light"
            style="margin-top: 10px;">
            😀
            </button>
            <div v-if="showEmojiTitle" style="margin-top: 10px;">
              <picker
                :include="['people']"
                :showSearch="false"
                :showPreview="false"
          ...
          <b-form-group label='正文'>
            <MdEditor v-model="posts.content"></MdEditor>
            <button variant='primary'
            @click="showEmojiOnContent()"
            class="btn btn-light"
            style="margin-top: 10px;">
            😀
            </button>
            <div v-if="showEmojiContent" style="margin-top: 10px;">
              <picker
                :include="['people']"
                :showSearch="false"
                :showPreview="false"
...
export default {
  ...
  data() {
    return {
      options: [], // tag标签选择列表
      uploadPhotosActionURL: 'https://ssemarket.cn/api/auth/uploadPhotos',
      fileList: [],
      dialogImageUrl: '',
      dialogVisible: false,
      posts: {
