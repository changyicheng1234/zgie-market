...
const modifyPassword = ({
  email, password, password2, valiCode,
}) => {
  return request.post('auth/modifyPassword', {
    email, password, password2, valiCode,
  });
};
...
// 已登录后的身份验证，DEPRECATED
const identityValidate = ({ email, valiCode, mode }) => {
  return request.post('auth/identityValidate', { email, valiCode, mode });
};

// 注销用户
const deleteUser = ({ phone, email, valiCode }) => {
  return request.post('auth/deleteMe', { phone, email, valiCode });
};
