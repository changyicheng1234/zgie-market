...
嵌套主题右侧显示只读的阅读进度：总数使用 Discourse 的真实帖子总数，当前位置按嵌套页面上的实际阅读顺序计算。这样较晚发布、但被归回早期一级留言下的二级回复不会让进度先前进再倒退；真实 `post_number` 仍用于帖子身份和回复关系。桌面空间足够时显示日期与竖向轨道，窄屏或右侧空间不足时缩为右下角计数浮块。
