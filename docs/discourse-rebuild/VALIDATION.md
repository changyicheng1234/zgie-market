...
| 单次匿名发布 | 新主题与回复 Composer 均提供匿名开关；只改变本次作者，不改变登录 Session，下一次默认关闭 |
| 匿名草稿清理 | 匿名主题发布后同步清理真实账号草稿，并使在途旧序列自动保存失效 |
| 会话匿名入口 | 用户菜单不再显示匿名切换；真实账号调用原生切换接口返回 403，旧匿名 Session 可退回真实账号 |
| 匿名身份入口 | 匿名头像、用户名不含资料链接；普通成员不能访问匿名资料 |
| 内容长度 | 标题、首帖、回复只禁止空内容；正文上限为 Discourse 支持的 150,000 字符 |
| 点赞与反应隐私 | 保留计数与 Emoji；不弹出用户列表，普通成员名单 API 返回 403，管理员可审计 |
| 右侧阅读进度 | 桌面显示日期、轨道及 `当前阅读位置 / 帖子总数`；嵌套回复不会让位置倒退 |
| 富文本 cooked HTML | 标题、粗体、斜体、引用、代码块、表格、链接与图片均已生成 |
| 图片资源 | 本地头像、Emoji、正文图片均返回 HTTP 200，烘焙 URL 使用 `localhost:4200` |
| 上游 lint | RuboCop、ESLint、Prettier、Stylelint 全部通过 |
| 插件测试 | 22 examples，0 failures（含真实浏览器系统测试） |
...
浏览器系统测试实际执行了以下流程：登录用户进入嵌套主题，点击二级留言的回复按钮并提交回复，页面最终在视觉深度 1 找到新内容，并确认视觉深度 0 不存在该内容；随后确认一级留言没有折叠控件、二级留言显示真实回复对象，并实际完成二级留言的收起与重新展开。测试在回复 Composer 中点击用户可见的“匿名回复”文字，并确认开关的 `aria-checked` 从 `false` 立即变为 `true`，随后用单字正文成功发布；刷新后确认头像和用户名均无资料入口，再次回复时确认开关已恢复关闭并以真实账号发布。测试还确认新主题 Composer 有“匿名发帖”开关、用户菜单不存在匿名切换入口；等待匿名新主题确实自动保存为 `new_topic_*` 草稿后完成发布，再进入真实账号草稿页确认其为空。测试创建反应后确认 Emoji 与计数可见、名单不可展开；最后构造一条原始楼层号较大、但被插回早期一级留言下的二级回复，确认继续向下阅读后右侧位置仍从 `3 / 10` 单调前进到 `4 / 10`，不会按原始楼层号倒退。

开发站浏览器实测结果：主题初始显示 `1 / 11`，进度轨道位于正文右侧，并按页面实际阅读顺序前进；反应区域渲染两个 Emoji 摘要，`pointer-events=none`，强制触发点击也没有出现参与者弹窗。
